...
from recurrent_ppo.pusht_gym import VecAugmentationDraw, build_vec_env, delta_scale_from_demos, env_kwargs_from
...
def train(args_cli, arch):
    ...
    def with_draw(venv, seed):
        ...
        return VecAugmentationDraw(venv, seed=seed, **aug) if aug else venv

    # create the vectorised pushT environment
    env = with_draw(arch.wrap(build_vec_env(
    ...
    if args_cli.video_freq > 0:
        ...
        callbacks.append(RolloutVideo(env_kwargs, args_cli.obs, args_cli.video_freq,
                                      ...
                                      n_stack=arch.video_n_stack(cfg), aug=aug))
    if args_cli.eval_freq > 0:
        ...
        eval_env = with_draw(arch.wrap(build_vec_env(
            obs_type=args_cli.obs,
            n_envs=1,
        ...
        if args_cli.eval_corrupt:
            if not args_cli.corrupt_obs:
                raise SystemExit("[ERROR] --eval-corrupt needs a policy that has corruption to turn "
                                 "on; this run was not started with --corrupt-obs.")
            callbacks.append(CorruptEvalCallback(
                eval_env, n_eval_episodes=args_cli.n_eval_episodes, eval_freq=eval_freq,
                eval_seed=args_cli.seed + 10_000, deterministic=True, verbose=1))
        second_env = with_draw(arch.wrap(build_vec_env(
            obs_type=args_cli.obs, n_envs=1, seed=args_cli.seed + 30_000,
            use_subproc=False, **second_kwargs), cfg), args_cli.seed + 30_000)
        if isinstance(env, VecNormalize):
...
def resolve_config(run_dir, args_cli, arch):
    ...
    if missing:
        print(f"[WARN] This run predates {len(missing)} key(s); assuming today's default for each. "
              f"See docs/reports/ppo/recurrent_ppo_runs.md for what it actually used.")
        for k in missing:
...
def play(args_cli, arch):
    ...
    # the corruption regime is the run's identity: the env must emit exactly what training did,
    ...
    if aug:
        env = VecAugmentationDraw(env, seed=args_cli.seed, **aug)
