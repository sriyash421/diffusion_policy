...
def add_common_args(parser):
    ...
    parser.add_argument("--q-net-arch", type=str, default=D["q_net_arch"],
                        help="Hidden sizes of the Q head, comma separated. It is a deliverable of the "
                             "clean arm, so it is configurable like everything else that is.")
    ...
    parser.add_argument("--eval-corrupt", action="store_true", default=False,
                        help="ALSO evaluate with the corruption on, under eval_corrupt/. For the "
                             "--corrupt-obs arm that is the number that says whether the POMDP was solved; "
                             "the clean eval/ series stays alongside it so the two are comparable.")
