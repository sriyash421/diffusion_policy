...
class MLPImagePolicy(BaseImagePolicy):
    def __init__(self,
            ...
            hidden_dim: int = 512,
            hidden_depth: int = 4,
            action_clip_range: float = None,  # NEW: Action clipping parameter
            **kwargs):
        ...
        # NEW: Action clipping configuration
        self.action_clip_range = action_clip_range  # None means no clipping
    ...
    def compute_loss(self, batch):
        ...
        # NEW: Apply action clipping if enabled
        if self.action_clip_range is not None:
            nactions = torch.clamp(nactions, 
