...
class QIteration:
    ...
    def __init__(self, maze):
        self.maze = maze
        self.num_states = maze.maze_map.shape[0] * maze.maze_map.shape[1]
    ...
    def state_to_cell(self, state):
        i = int(state / self.maze.maze_map.shape[1])
        j = state % self.maze.maze_map.shape[1]
        return (i, j)

    def cell_to_state(self, cell):
        return cell[0] * self.maze.maze_map.shape[1] + cell[1]

    def get_q_values(self, num_itrs=None, discount=0.99):
        if hasattr(self, 'q_values'):
            return self.q_values

        # One sweep propagates reward one cell, so fewer sweeps than the longest possible
        # path leaves distant states at q=0 and the policy picks an arbitrary action. The
        # old fixed 50 was silently enough for an 11x11 maze and not for a 23x23 one.
        if num_itrs is None:
            num_itrs = max(50, self.num_states)
    ...
    def _check_valid_cell(self, cell):
        # Out of map bounds
        if cell[0] >= self.maze.maze_map.shape[0]:
            return False
        elif cell[1] >= self.maze.maze_map.shape[1]:
            return False
        # Wall collision
...
class WaypointController:
    ...
    def __init__(self, maze, gains={"p": 10.0, "d": -1.0}, waypoint_threshold=0.1,
        ...
        # The waypoint dither below is what makes demonstrations diverse, so it stays --
        # but drawn from a passed-in rng it is reproducible from a seed. Default is the
        # global numpy stream, which is what the pointmaze datasets were collected with.
        self.rng = rng if rng is not None else np.random

    def compute_action(self, obs):
        # Check if we need to generate new waypoint path due to change in global target
        if (
            np.linalg.norm(self.maze.cur_goal_xy - obs["desired_goal"]) > 1e-3
            or self.waypoint_targets is None
        ):
            # Convert xy to cell id
            achieved_goal_cell = tuple(
                self.maze.xy_to_ij(obs["achieved_goal"])
            )
            self.global_target_id = tuple(
                self.maze.xy_to_ij(obs["desired_goal"])
            )
            ...
            if self.waypoint_targets:
                ...
                self.current_control_target_xy = self.maze.ij_to_xy(
    ...
    def compute_pi_star(self, obs):
        achieved_goal_cell = tuple(
            self.maze.xy_to_ij(obs["achieved_goal"])
        )
        global_target_id = tuple(
            self.maze.xy_to_ij(obs["desired_goal"])
        )
        global_target_xy = obs["desired_goal"]

        waypoint_targets = self.maze_solver.generate_path(
            achieved_goal_cell, global_target_id
        )

        # Check if the waypoint dictionary is empty
        # If empty then the ball is already in the target cell location
        if waypoint_targets:
            current_control_target_id = waypoint_targets[
                achieved_goal_cell
            ]
            current_control_target_xy = self.maze.ij_to_xy(
                np.array(current_control_target_id)
            )
        else:
            waypoint_targets[
                current_control_target_id
            ] = current_control_target_id
            current_control_target_id = global_target_id
            current_control_target_xy = global_target_xy

        # Check if we need to go to the next waypoint
        dist = np.linalg.norm(current_control_target_xy - obs["achieved_goal"])
        if (
            dist <= self.waypoint_threshold
            and current_control_target_id != global_target_id
        ):
            current_control_target_id = waypoint_targets[
                current_control_target_id
            ]
            # If target is global goal go directly to goal position
            if current_control_target_id == global_target_id:
                current_control_target_xy = global_target_xy
            else:
                current_control_target_xy = (
                    self.maze.ij_to_xy(
                        np.array(current_control_target_id)
                    )
                    - self.rng.uniform(size=(2,)) * 0.2
        ...
        action = (
            self.gains["p"] * (current_control_target_xy - obs["achieved_goal"])
            # + self.gains["d"] * obs["observation"][2:]
        )
        action = np.clip(action, -1, 1)
...
if __name__ == "__main__":
    import ogbench
    env = ogbench.make_env_and_datasets("pointmaze-medium-navigate-singletask-v0", env_only=True)
    controller = WaypointController(env.unwrapped, waypoint_threshold=.3)
    obs, info = env.reset()
    done = False
    terminated = False
    goal = info['goal']
    ...
    while not done and not terminated:
        obs_dict = {
            "observation": env.unwrapped.state_vector(),
            "achieved_goal": obs,
            "desired_goal": goal
        }
        action = controller.compute_action(obs_dict)
        obs, reward, done, terminated, info = env.step(action)
        frames.append(env.render())
    from onemodel.utils import save_video
    save_video(frames, "pointmaze_waypoint.mp4", fps=30)
