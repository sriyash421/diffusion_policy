# Latest PushT success rates

The current generation: the **30-demo search-vs-BC pair**, trained 2026-08-17/18 after the
search-procedure unification, evaluated after the 2026-08-18 selection fixes.

Everything superseded lives in `SUCCESS_RATES.md` — section C holds this generation's
pre-fix best-of-n tables (with why they were stochastic), section B the pre-2026-08-17
generation, section A older arms still.

`success = coverage >= 95%`; `mean reward = clip(coverage/0.95, 0, 1)` averaged over
episodes. Test is 50 episodes, val 30. Seed 42, single seed.

**Noise floor: ~0.07 mean reward / ~0.06 success rate.** Measured, not assumed — before the
fixes, argmax and softmax at n=1 ran the identical policy on different RNG streams, so the
gap between those columns was a direct read of run-to-run variation (section C in
`SUCCESS_RATES.md` has the numbers). Nothing below smaller than that is resolvable.

## The two arms

Both are `PushTDiffusionSearchPolicy` (DDIM at 8 steps) with a **byte-identical** obs
encoder and denoiser, verified from each run's `.hydra/config.yaml`: ResNet18
`IMAGENET1K_V1`, `crop_shape [76,76]`, `random_crop`, `use_group_norm`, `imagenet_norm:
False`; 4 layers / 2 cond layers / 4 heads / 256 emb, `p_drop_attn 0.2`, `causal_attn:
True`, `cond_encoder: gpt2`. **The only difference is `max_actions`: 16 vs 1.**

"SEARCH" means search width 16 — not the `train_pusht_st_n1` arm, which is a separate
width-1 config with `num_inference_steps: 100` and was not part of these runs. The Gaussian
arm (`train_pusht_gaussian_search`) exists in the tree but has never been run.

| | SEARCH (k=16) | BC (k=1) |
|---|---|---|
| config | `train_pusht_diffusion_search n_demos=30` | `train_pusht_bc n_demos=30` |
| `max_actions` | 16 | 1 |
| trainer | `outer_inner` | `offline` |
| run dir | `outer_inner/value_k16_corrupt-False_demos-30_seed-42` | `offline/bc_demos-30_seed-42` |
| wandb | `5ufduqb2` | `mzatxyrb` |
| wall clock | 12h26m | 44m |
| final train / val loss | 0.0093 / 0.3174 | 0.0212 / 0.2005 |

Split: `pusht_seed42_train30.json` — 30 train episodes, a strict subset of `train100`'s
train list, reusing its val (30) and test (50) verbatim, so 30-vs-100 isolates dataset size.

## 1. In-training rollouts (mean reward, max)

Fired by the trainer at 20k-step intervals, on **EMA** weights. Argmax only, so **both
2026-08-18 fixes leave these untouched** — neither the selection generator nor the n
semantics affects an argmax rollout.

**Not a matched comparison**: SEARCH ran at n=16 (search active), BC at n=1. Section 2 is
the like-for-like reading.

| arm | 20k | 40k | 60k | 80k | 100k |
|---|---|---|---|---|---|
| SEARCH test | 0.7208 | 0.7755 | 0.8023 | 0.745 | 0.6143 |
| SEARCH val  | 0.8143 | 0.7574 | 0.7539 | 0.7708 | 0.7362 |
| BC test  | 0.4957 | 0.612 | 0.6029 | 0.6382 | - |
| BC val   | 0.4577 | 0.5681 | 0.6342 | 0.6397 | - |

BC's 100k row is blank because **the offline trainer skips its final rollout**: its rollouts
drift +1 per fire (20001, 40002, 60003, 80004) so the next trigger lands at 100005, past the
`max_gradient_steps: 100000` cap. Evaluated separately from `step_0100000.ckpt`: test mean
reward 0.593, success 0.240. `TrainSearchOuterInnerWorkspace` hits its boundaries exactly
and does not have this bug.

**Both arms are over-trained at 100k.** SEARCH peaks at 60k and falls 19pp by 100k; BC peaks
at 80k. 60-80k would have been the right budget at this demo count.

**Open discrepancy:** the same SEARCH checkpoint at n=16 scored **0.8023** here but **0.724**
in the standalone sweep. Same weights, same n. Most likely EMA vs raw weights — these
rollouts use the EMA model and `eval_search_pusht.py` may not — but unconfirmed. Until it is
resolved, treat this table and section 2 as measuring different things.

<!-- BEGIN grid (generated by scripts/update_latest_with_grid.py) -->

## 2. Best-of-n grid -- policy x selection x checkpoint

Every `step_*.ckpt` on the 10k grid x {argmax, softmax, final_pass} x n in {1,2,4,8,16}, both arms. **13/60 combos complete -- sweep still running, table is partial.**

Pick a checkpoint on **val** and read **test** at the same row; this grid exists so a checkpoint never has to be pre-selected. Selecting by maximising test inflates the reported number by 1-2 SE.

**n counts GENERATIONS, identically under all three rules**, so the columns are matched on compute. At n=1 there is nothing to select, so all three rules run the same empty-context conditional and their n=1 cells must agree exactly -- a disagreement there is a bug, not a result.

### TEST  --  reward

| policy | selection | checkpoint | n=1 | n=2 | n=4 | n=8 | n=16 |
|---|---|---|---|---|---|---|---|
| SEARCH | argmax | 10,000 | 0.486 | 0.634 | 0.757 | 0.801 | 0.797 |
|  |  | 20,000 | 0.570 | 0.661 | 0.713 | 0.718 | 0.727 |
|  |  | 30,000 | 0.600 | 0.693 | 0.709 | 0.759 | 0.775 |
|  |  | 40,000 | 0.576 | 0.666 | 0.676 | 0.724 | 0.739 |
|  | softmax | 10,000 | 0.486 | 0.668 | 0.695 | 0.724 | 0.697 |
|  |  | 20,000 | 0.570 | 0.635 | 0.667 | 0.677 | 0.683 |
|  |  | 30,000 | 0.600 | 0.685 | 0.645 | 0.735 | 0.698 |
|  |  | 40,000 | 0.576 | 0.654 | 0.696 | 0.693 | 0.690 |
|  | final_pass | 10,000 | 0.486 | 0.616 | 0.611 | 0.674 | 0.666 |
|  |  | 20,000 | 0.570 | 0.668 | 0.666 | 0.640 | 0.652 |
|  |  | 30,000 | 0.600 | 0.644 | 0.644 | 0.683 | 0.654 |
|  |  | 40,000 | 0.576 | 0.621 | 0.656 | 0.676 | 0.682 |

### TEST  --  success

| policy | selection | checkpoint | n=1 | n=2 | n=4 | n=8 | n=16 |
|---|---|---|---|---|---|---|---|
| SEARCH | argmax | 10,000 | 0.060 | 0.160 | 0.320 | 0.360 | 0.400 |
|  |  | 20,000 | 0.100 | 0.240 | 0.180 | 0.340 | 0.320 |
|  |  | 30,000 | 0.120 | 0.220 | 0.200 | 0.260 | 0.300 |
|  |  | 40,000 | 0.300 | 0.260 | 0.300 | 0.360 | 0.300 |
|  | softmax | 10,000 | 0.060 | 0.060 | 0.200 | 0.100 | 0.240 |
|  |  | 20,000 | 0.100 | 0.120 | 0.200 | 0.220 | 0.340 |
|  |  | 30,000 | 0.120 | 0.220 | 0.140 | 0.220 | 0.260 |
|  |  | 40,000 | 0.300 | 0.220 | 0.340 | 0.260 | 0.240 |
|  | final_pass | 10,000 | 0.060 | 0.080 | 0.120 | 0.100 | 0.160 |
|  |  | 20,000 | 0.100 | 0.180 | 0.140 | 0.140 | 0.140 |
|  |  | 30,000 | 0.120 | 0.200 | 0.160 | 0.220 | 0.160 |
|  |  | 40,000 | 0.300 | 0.160 | 0.200 | 0.180 | 0.160 |

### VAL  --  reward

| policy | selection | checkpoint | n=1 | n=2 | n=4 | n=8 | n=16 |
|---|---|---|---|---|---|---|---|
| SEARCH | argmax | 10,000 | 0.478 | 0.713 | 0.796 | 0.763 | 0.788 |
|  |  | 20,000 | 0.494 | 0.741 | 0.707 | 0.808 | 0.785 |
|  |  | 30,000 | 0.641 | 0.768 | 0.750 | 0.709 | 0.794 |
|  |  | 40,000 | 0.687 | 0.725 | 0.721 | 0.726 | 0.745 |
|  | softmax | 10,000 | 0.478 | 0.666 | 0.756 | 0.699 | 0.758 |
|  |  | 20,000 | 0.494 | 0.651 | 0.729 | 0.710 | 0.805 |
|  |  | 30,000 | 0.641 | 0.783 | 0.715 | 0.769 | 0.764 |
|  |  | 40,000 | 0.687 | 0.653 | 0.665 | 0.758 | 0.715 |
|  | final_pass | 10,000 | 0.478 | 0.653 | 0.715 | 0.633 | 0.662 |
|  |  | 20,000 | 0.494 | 0.715 | 0.685 | 0.755 | 0.709 |
|  |  | 30,000 | 0.641 | 0.677 | 0.688 | 0.762 | 0.738 |
|  |  | 40,000 | 0.687 | 0.655 | 0.722 | 0.720 | 0.720 |

### VAL  --  success

| policy | selection | checkpoint | n=1 | n=2 | n=4 | n=8 | n=16 |
|---|---|---|---|---|---|---|---|
| SEARCH | argmax | 10,000 | 0.067 | 0.067 | 0.233 | 0.467 | 0.333 |
|  |  | 20,000 | 0.033 | 0.200 | 0.300 | 0.333 | 0.400 |
|  |  | 30,000 | 0.100 | 0.233 | 0.367 | 0.267 | 0.367 |
|  |  | 40,000 | 0.067 | 0.167 | 0.367 | 0.300 | 0.300 |
|  | softmax | 10,000 | 0.067 | 0.100 | 0.133 | 0.200 | 0.233 |
|  |  | 20,000 | 0.033 | 0.233 | 0.133 | 0.167 | 0.267 |
|  |  | 30,000 | 0.100 | 0.367 | 0.200 | 0.267 | 0.233 |
|  |  | 40,000 | 0.067 | 0.233 | 0.233 | 0.233 | 0.267 |
|  | final_pass | 10,000 | 0.067 | 0.133 | 0.200 | 0.100 | 0.100 |
|  |  | 20,000 | 0.033 | 0.333 | 0.200 | 0.100 | 0.100 |
|  |  | 30,000 | 0.100 | 0.100 | 0.100 | 0.200 | 0.167 |
|  |  | 40,000 | 0.067 | 0.233 | 0.067 | 0.167 | 0.133 |

Regenerate: `python scripts/update_latest_with_grid.py`

<!-- END grid -->

## 3. Where the raw results are

| what | path (under `$DP_OUTPUT_ROOT/pusht_search/pusht_image_search`) |
|---|---|
| grid curves + per-episode rewards | `bon_grid_30demo/{search,bc}/step_00*/success_curve.json` |
| all selection modes merged | `bon_grid_30demo/{search,bc}/success_curves.jsonl` |
| per-candidate verifier distances | `bon_grid_30demo/*/step_00*/candidate_scores.jsonl` — one row per policy call, so any selection rule can be replayed offline without re-running the sim |
| grid stdout | `logs/bon_grid_30demo.log` (repo) |
| in-training metrics | `{outer_inner,offline}/*/logs.json.txt` |
| resolved configs | `{outer_inner,offline}/*/.hydra/config.yaml` |
| pre-fix grid + sweep (provenance for `SUCCESS_RATES.md` section C; do not quote) | `bon_grid_30demo.pre-nfix-*/`, `bon_30demo/`, `logs/bon_grid_30demo.log.pre-nfix-*`, `logs/bon_sweep_30demo.log` |

Reproduce: `bash scripts/run_30demo_2x2.sh` then `bash scripts/bon_grid_30demo.sh`.
